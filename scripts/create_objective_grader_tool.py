#!/usr/bin/env python3
"""Create or update the objective auto-grader tool in Open WebUI.
...
DEFAULT_TOOL_ID = "objective_grader"
...
def build_content() -> str:
    ...
    if "class Tools:" not in text or "def grade_from_bank(" not in text or "def grade_answers(" not in text:
        raise RuntimeError("工具源码缺少 Tools.grade_from_bank / grade_answers 定义")
...
def payload(tool_id: str, name: str, content: str, bank_size: int) -> dict[str, Any]:
    return {
        ...
        "name": name,
        "content": content,
        "meta": {"description": f"客观题自动判分：基于真实题库（{bank_size} 题，来源 knowledge/ 资料）结构化判分并输出解析"},
        "access_grants": [],
    }
